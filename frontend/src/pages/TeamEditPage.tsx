...
const emptyRunnerForm: RunnerFormState = {
  name: "",
  leg: "1",
  bib: "",
  sex: "",
...
function TeamEditPage() {
  ...
  function handleSaveTeam(e: FormEvent) {
    ...
    setTeamSaving(true);
    setTeamError(null);
    apiRequest(`/api/teams/${teamId}`, "PUT", {
      name: teamName,
  ...
  function handleDeleteTeam() {
    ...
    if (
      !window.confirm(
        "This will permanently delete this team and all of its runners. Continue?",
      )
    )
      return;
  ...
  function handleSetRunnerStatus(
    runnerId: number,
    ...
    setRunnerRowError(null);
    apiRequest(`/api/teams/${teamId}/runners/${runnerId}/status`, "PATCH", {
      status,
    })
      .then(() => loadTeam())
      .catch((err: Error) => setRunnerRowError(err.message))
  ...
  function handleDeleteRunner(runnerId: number) {
    ...
    if (!window.confirm("This will permanently delete this runner. Continue?"))
      return;
                        ...
                        editingRunnerId === runner.id ? (
                          <tr key={runner.id}>
                            ...
                            <td>
                              <button
                                ...
                                className={
                                  runner.status === "ACTIVE"
                                    ...
                                    : "status-toggle status-toggle-inactive"
                                }
                                onClick={() =>
                                  handleSetRunnerStatus(
                                    runner.id,
                                    runner.status === "ACTIVE"
                                      ? "INACTIVE"
                                      : "ACTIVE",
                              ...
                              >
                                {runnerStatusUpdatingId === runner.id
                                  ? "Updating…"
                                  : runner.status}
                              </button>
                            </td>
                            ...
                            <td>
                              <button
                                ...
                                disabled={
                                  ...
                                  runnerDeletingId !== null ||
                                  runnerStatusUpdatingId !== null
                                }
                              >
                                {runnerStatusUpdatingId === runner.id
                                  ? "Updating…"
              ...
              >
                <label className="form-field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddRunnerForm((f) => ({ ...f, name: e.target.value }))
                    }
                    required
                ...
                <label className="form-field form-field-narrow">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddRunnerForm((f) => ({ ...f, leg: e.target.value }))
                    }
                    required
                  />
                </label>
                <label className="form-field form-field-narrow">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddRunnerForm((f) => ({ ...f, bib: e.target.value }))
                    }
                    required
                    placeholder="A01"
                  />
                ...
                {addRunnerError && (
                  <p className="form-error">{addRunnerError}</p>
                )}
                <button
                  type="submit"
                  className="start-button"
                  disabled={addRunnerPending}
                >
